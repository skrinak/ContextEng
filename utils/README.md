...
| `fsi_links.py` | Fetches every address the corpus cites. Fails only when an address recorded as working is now dead; bot-blocked sites are reported, not failed. Set `FSI_LINKCHECK_CONTACT` to a monitored contact address, because sec.gov refuses requests without one. CI reads it from the repository secret of the same name. `make fsi-links` |
